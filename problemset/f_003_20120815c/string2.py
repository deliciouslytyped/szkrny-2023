...
def not_bad(s):
    try:
        if (end := s.index("bad")) > (start := s.index("not")):
            return s[:start] + "good" + s[end+3:]
        else:
            return s
    except ValueError:
        return s
...
def front_back(a, b):
    split = lambda s: (s[:len(s)//2+1], s[len(s)//2+1:]) if len(s) % 2 else (s[:len(s)//2], s[len(s)//2:])
    (a, b), (c, d) = split(a), split(b)
    return a + c + b + d
